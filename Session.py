...
class GameSession:
    ...
    def __init__(self, players, send_message, max_turn_duration_sec: int = 90):
        ...
        for elem in self.players:
            if elem.name[:3] == 'Bot':
                self.bots.append(Bot(bot_id=elem.client_id))
    ...
    def desk_state_message(self, player):
        ...
        for elem in self.bots:
            if player.client_id == elem.bot_id:
                elem.on_message(message={'type': 'DeskState',
                                         'desk': [list(row) for row in self.desk.desk]})
    ...
    def hand_state_message(self, player):
        ...
        for elem in self.bots:
            if player.client_id == elem.bot_id:
                elem.on_message(message={'type': 'HandState',
                                         'hand': [str(card) for card in player.hand.cards]})
    ...
    async def on_message(self, client_id: str, message: dict):
        ...
        if message['type'] == 'IPutCard':
            ...
            if player.hand.get_amount() == 0 and self.pack.length() == 0:
                for player in self.players:
                    self.send_message(
                        client_id=player.client_id,
                        message={
            ...
            for i in range(player.hand.get_amount()):
                ...
                self.pack.add_card(card)  # кладём карту в колоду
            self.pack.shuffle()  # перемешиваем колоду
            # добавляем 4 карты в руку игроку
            if self.pack.length() >= 4:  # если в колоде осталось больше 4 карт
                for i in range(4):
        ...
        # игрок решил покинуть игру, при этом все его карты остаются на столе,
        # карты с его руки добавляются в колоду, последняя перемешивается
